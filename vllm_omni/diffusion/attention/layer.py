...
class Attention(nn.Module):
    ...
    def _assert_metadata_compatible(self, attn_metadata: AttentionMetadata | None) -> None:
        """Reject metadata the resolved backend cannot express at all.

        A mask the backend does not read at all is an error here, not a reroute:
        only a backend that claims mask support and merely lacks the *dense*
        variant is sent to SDPA by ``_mask_needs_sdpa``.
        """
        if attn_metadata is None or self.attn_backend is None:
            return
        if attn_metadata.attn_mask is not None and not self.attn_backend.supports_attention_mask(
            getattr(self, "attn_spec", None)
        ):
            raise ValueError(
                f"Attention backend '{self.attn_backend.get_name()}' does not support attn_mask. "
                "Select a mask-capable backend."
            )
